  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <div className="rounded-3xl border border-zinc-800 bg-zinc-900/70 p-6 animate-pulse">
          <div className="h-20 w-20 bg-zinc-800 rounded-full mb-4"></div>
          <div className="h-6 w-1/3 bg-zinc-800 rounded mb-2"></div>
  ...
  if (error || !profileData) {
    return (
      <div className="rounded-3xl border border-rose-500/30 bg-rose-500/10 p-6 text-rose-300 text-center">
        ...
        <Link to="/dashboard" className="text-white underline mt-4 inline-block">
    ...
    <div className="space-y-6">
      <div className="rounded-3xl border border-zinc-800 bg-zinc-900/70 p-6 shadow-2xl shadow-black/20">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div className="pt-1">
              <div className="flex items-center gap-2">
                ...
                {followsMe && !isMe && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-zinc-800 px-2 py-0.5 text-[10px] font-medium text-zinc-300 border border-zinc-700">
                    Follows you
                  </span>
                )}
              </div>
          ...
          <div className="shrink-0 flex flex-col items-end">
            {isMe ? (
              <Link to="/settings">
                <Button variant="ghost" className="border border-zinc-700 hover:border-emerald-500 hover:text-emerald-400 text-sm">
                  Edit Profile
                </Button>
              </Link>
            ) : (
              <Button
                ...
                className={`flex items-center gap-2 text-sm ${
                  isFollowing
                    ? "border border-zinc-700 hover:border-rose-500 hover:text-rose-400 hover:bg-rose-500/10"
          ...
          {statsCards.map((item) => {
            ...
            return (
              <div
                ...
                className={`rounded-2xl border border-zinc-800 bg-zinc-950/50 p-4 text-center transition-colors ${
                  isClickable ? "cursor-pointer hover:bg-zinc-900" : ""
                }`}
              >
      ...
      {connectionsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 py-6 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl border border-zinc-800 bg-zinc-900 shadow-2xl shadow-black/40 overflow-hidden flex flex-col max-h-[80vh]">
                ...
                <div className={connectionsStyles.userList}>
                  {connections[connectionsModal].map((connUser) => (
                    <div key={connUser._id} onClick={() => setConnectionsModal(null)}>
                      <UserBadge 
                        user={connUser} 
                        className="rounded-xl px-2 py-2 hover:bg-zinc-800/60 transition-colors w-full" 
                      />
                    </div>
                  ))}
                </div>
      ...
      <div>
        <h2 className="text-lg font-semibold text-white mb-4 px-2">Polls by @{user.username}</h2>
